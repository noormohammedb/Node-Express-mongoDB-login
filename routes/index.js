...
const dbUrl = `mongodb+srv://${username}:${password}@expresslogin.pwvce.mongodb.net/${database}?retryWrites=true&w=majority`;
...
router.post('/signup', (req, res, next) => {
  ...
  MongoClient.connect(dbUrl,{useUnifiedTopology:true}, (error, client) => {
    ...
    if (error) {
      ...
      client.db(database).collection('users').insertOne(req.body,(error,data)=>{
        if(error){
...
router.post('/login',(req,res,next) => {
  ...
  MongoClient.connect(dbUrl,{useUnifiedTopology:true},(error,client)=>{
    if(error){
      ...
      client.db(database).collection('users').findOne({email:req.body.email},(error,data)=>{
        if(error){
